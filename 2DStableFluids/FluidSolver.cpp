...
n(60), size(n*n), h(0.1), laplacian(size,size), diffusion(size,size), velDiff(size, size)
{
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			...
			if (i-1>0) {
				...
				velDiff.set1Value(index, index - 1, -1. * velDiff_coef);
				count++;
			}
			if (i+1<n-1) {
				diffusion.set1Value(index, index+1, -1.0*diffusion_coef);
			...
			if (j-1>0) {
				...
				velDiff.set1Value(index, index - n, -1. * velDiff_coef);
				count++;
			}
			if (j+1<n-1) {
				diffusion.set1Value(index, index+n, -1.0*diffusion_coef);
			...
			velDiff.set1Value(index, index, 1.+count*velDiff_coef);
		}
	}
...
void CFluidSolver::updateVelocity()
{
	...
	double* vec_x = new double[size];
	double* vec_y = new double[size];
	double* vel_x = new double[size];
	double* vel_y = new double[size];

	for (int i = 0; i < size; i++) {
		vec_x[i] = advected_velocity[i].x;
		vec_y[i] = advected_velocity[i].y;
		vel_x[i] = velocity[i].x;
		vel_y[i] = velocity[i].y;
	}

	velDiff.solve(vec_x, vel_x, 1e-8, 30);
	velDiff.solve(vec_y, vel_y, 1e-8, 30);

	for (int i = 0; i < size; i++) {
		//auto l = velocity_source[i];
		//auto m = vec_x[i];
		velocity[i].x = vec_x[i];
		velocity[i].y = vec_y[i];
		auto zell = false;
	}
...
void CFluidSolver::velocity_advection()
{
	...
	for (int i = 1; i < n-1; i++)
		for (int j=1; j < n-1; j++) {
			...
			//vec2 v = vec2(i, j) + velocity[i + j * n] * h * (-1);
			...
			advected_velocity[i + j * n] = velocity[i0 + j0 * n] * (1 - s) * (1 - t) + velocity[i0 + (j0 + 1) * n] * (1 - s) * t + velocity[(i0 + 1) + j0 * n] * s * (1 - t) + velocity[(i0 + 1) + (j0 + 1) * n] * s * t;

			currD = density[i + j * n];
			buoyForce = vec2(0, -1) * 0.1 * currD;

			advected_velocity[i + j * n] = advected_velocity[i + j * n] + buoyForce;
		}
}

void CFluidSolver::incVis()
{
	if (velDiff_coef >= 22.)
		return;
	reset();
	...
	velDiff_coef += 4;
	velDiff.setDimensions(size, size);
